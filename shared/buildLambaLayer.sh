...
curl --header "JOB-TOKEN: $CI_JOB_TOKEN" --upload-file ${PARENT_PATH}/shared_python/dist/"${FILE_NAME_LAYER} "${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/${FILE_NAME_LAYER}/${VERSION}/${FILE_NAME_LAYER}"
curl --header "JOB-TOKEN: $CI_JOB_TOKEN" --upload-file ${PARENT_PATH}/shared_python/dist/"${FILE_NAME_LAYER} "${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/${FILE_NAME_LAYER}/latest/${FILE_NAME_LAYER}"
rm -rf ${PARENT_PATH}/shared_python/dist